...
if (isset($_GET['id'])) {
    ...
    $ambil = caridata($mysqli,"SELECT gambar FROM izin WHERE id_izin = '".$_GET['hapus']."'");
    $stmt = $mysqli->prepare("DELETE FROM izin where id_izin=?");
    $stmt->bind_param("s",mysqli_real_escape_string($mysqli, $_GET['hapus'])); 

        if ($stmt->execute()) { 

        //HAPUS FILE
        if(is_file("../gambar/izin/".$ambil) && $ambil!='default.jpg')
        {
            unlink("../gambar/izin/".$ambil);
            unlink("../gambar/izin/small_".$ambil);
        }
