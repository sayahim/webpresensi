...
if(isset($_POST['tambah']))
{
	...
	$stmt = $mysqli->prepare("DELETE FROM alpha where id_alpha=?");
	$stmt->bind_param("s",mysqli_real_escape_string($mysqli, $_GET['hapus'])); 

	if ($stmt->execute()) { 
		echo "<script>alert('Data Alpha Berhasil Dihapus')</script>";
		echo "<script>window.location='index.php?hal=alpha';</script>";	
	} else {
		echo "<script>alert('Data Alpha Gagal Dihapus')</script>";
		echo "<script>window.location='javascript:history.go(-1)';</script>";
	}
